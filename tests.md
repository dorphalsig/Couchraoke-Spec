...
- **Phone**: header-only scan → produces `/manifest.json` `SongEntry` metadata (`isValid`, `title`, `artist`, `isDuet`, `hasRap`, `canMedley`, asset URLs)
...
> Produces `/manifest.json` `SongEntry` entries from header-only `.txt` scan.
...
- Replace WebSocket song-list exchange with HTTP `GET /manifest.json` after each successful `hello`
...
| 8.1.3 | TV fetches `/manifest.json` after `sessionState` | Manifest fetch occurs immediately after hello |
...
### 8.2 Manifest handling [U]

| # | What | Expected |
|---|---|---|
| 8.2.1 | Phone A manifest fetched → library updated | Songs attributed to `clientId=A` visible |
| 8.2.2 | Phone A disconnects | All songs for `clientId=A` removed immediately |
| 8.2.3 | Rescan updates manifest | Re-fetch replaces all prior entries for that `clientId` (not appended) |
...
| 10.1.5 | Manifest re-fetch replaces, not appends | Old entries for `clientId` gone |
...
| F15 | **Update**: add `httpPort`, `connectionId`; remove `assignPlayer`; replace WebSocket song-list exchange with HTTP manifest fetch |
