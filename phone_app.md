...
Random token, minimum 32 bits entropy (recommended 64+). Same token shown as join code AND used as `token` query parameter. Human-enterable: case-insensitive alphabet, MAY display in groups (e.g., `ABCD-EFGH`). Phone normalization: strip spaces/hyphens, case-insensitive comparison. Generated per session; reuse across sessions NOT allowed.

### QR Payload Format (Normative)

Encodes the full WebSocket endpoint URL: `ws://<tv-ip>:<ws-port>/?token=ABCDEFGH`.
