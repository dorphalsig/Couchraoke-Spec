# Couchraoke TV Host Specification
Version 4.20 (TV Host)
Status: Draft

## Table of Contents
1. Introduction & Product Contract
2. Architecture & Data Responsibilities
3. Song Library & Index
4. Parsing and Validation
5. Timing & Beat Model
6. Scoring
7. Multiplayer & Session Lifecycle
8. Network Protocol
9. TV UI Screens & Flows
10. Results
11. Appendices
...
## 2. Architecture & Data Responsibilities (Legacy §2)
...
- Android implementation: see the Android companion specification §3.1.1.1.
- iOS implementation: see the iOS companion specification §3.1.1.2.
...
#### 3.1.3 TV-side library (Legacy §3.1.3)
...
#### 3.1.4 TV-side song file lifecycle (Legacy §3.1.4)
...
### 3.3 Index Fields (Functional) (Legacy §3.3)
**Tests:** F01, F04, F16
...
### 3.4 Song List (Landing Screen) (Legacy §3.4)
**Tests:** none
...
### 3.5 Advanced Search (Overlay) [POST-MVP] (Legacy §3.5)
**Tests:** none
...
## 4. Parsing and Validation (Legacy §4)
...
## 5. Timing & Beat Model (Legacy §5)

## 5.1 Authoritative Beat Definitions (Legacy §5.1)
**Tests:** F06
...
## 5.2 Pitch Frame Timing, Jitter, and Mic Delay (Legacy §5.2)
**Tests:** F13, F14v2
...
## 5.3 Beat-Time Conversion (Legacy §5.3)
**Tests:** F06
...
## 6.1 Scoring Overview (Legacy §6.1)
**Tests:** F08, F09
...
## 6.2.1 ScoreFactor constants (Legacy §6.2.1)
**Tests:** F11
...
## 6.3 Player Level / Tolerance (Legacy §6.3)
**Tests:** F09
...
## 6.4 Octave Normalization (Legacy §6.4)
**Tests:** F09
...
## 6.6 Rounding and Display (Legacy §6.6)
**Tests:** F11
...
## 7. Multiplayer, Pairing, and Session Lifecycle (Legacy §7)

## 7.1 Session States (Legacy §7.1)
**Tests:** F15
...
## 7.3 Pairing UX (Phone) (Legacy §7.3)
**Tests:** none
...
## 7.4 Disconnect/Reconnect (Legacy §7.4)
**Tests:** F15
...
## 8. Network Protocol (Legacy §8)

## 8.1 Transport (Legacy §8.1)
**Tests:** F18, F14v2
...
## 8.2 Control Messages (Legacy §8.2)
**Tests:** F15, F18
...
## 8.3 Pitch Stream Messages (Legacy §8.3)
**Tests:** F12v2
...
## 8.4 Versioning and Compatibility (Legacy §8.4)
**Tests:** none
...
## 8.5 Sender Identification (Legacy §8.5)
**Tests:** F15
...
## 8.6 Song File HTTP Server (Legacy §8.6)
**Tests:** F18, F19
...
## 8.7 Time Sync and Jitter Handling (Legacy §8.7)
**Tests:** F14v2, F13
...
## 9. UI Screens and Flows (Legacy §9)
...
### 9.5.1 Singing Screen (Medley mode) (Legacy §9.5.1)
...
## 9.6 Results (Legacy §9.6)
**Tests:** F16


### 9.6.1 Results (post-song) (Legacy §9.6.1)
Show per singer:
...
### 9.6.2 Results (post-medley) (Legacy §9.6.2)
...
## 10. Results (Legacy §10)

### 10.1 Post-song flow (TV results screen) (Legacy §9.6.1)
Show per singer:
- Notes score, Golden score, Line bonus, **Song Total** (tens-rounded per USDX rules).
Actions:
- MVP has no persistent song queue; returning to Song List is required to start another song. The Song List screen may maintain a transient Medley playlist (Section 3.4) that is initialized when the screen is shown.
...
### 10.2 Medley results flow (Legacy §9.6.2)
After a medley run finishes, show a single results screen with a static score table listing each segment score and the aggregate Medley Total. No Left/Right navigation between rounds is required.
...
| QR code scanning | `AVFoundation` + `AVCaptureMetadataOutput` (platform API) | n/a — SDK built-in | `AVCaptureMetadataOutput` with `metadataObjectTypes = [.qr]`. No third-party dependency. |
| LAN discovery (mDNS) | `Network.framework` `NWBrowser` (platform API) | n/a — SDK built-in (iOS 13+) | Browse `_karaoke._tcp.` using `NWBrowser`. Modern replacement for deprecated `NSNetServiceBrowser`. |
