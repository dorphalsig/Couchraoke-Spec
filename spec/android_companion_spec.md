# Couchraoke Android Companion Specification
Version 4.20 (Android Companion)
Status: Draft
...
- Platforms: Android TV host app (Kotlin) + Android companion app (Kotlin) acting as mic client / song source alongside iOS (see separate spec).
...
- **#INSTRUMENTAL / #VOCALS handling (normative for phone metadata):** When `#INSTRUMENTAL` is present and the referenced file exists under the songs folder, the Android app MUST resolve its SAF URI (see §3.1.1.1) and expose it through the HTTP server + metadata payload. `#VOCALS` is emitted only if both the tag exists and file exists. Missing files are logged (see §4.3) and omitted from URLs.
...
- Songs stored on connected phones in a single songs folder per phone; TV aggregates library from all connected phones. The Android phone runs a lightweight read-only HTTP server for the duration of the session; the TV fetches song files directly over HTTP on demand (Section 7). No temporary storage or background syncing on the TV is required.
...
- **Android Companion App**: song storage (single songs folder on the phone), song metadata scanning, lightweight read-only HTTP file server for song asset delivery, mic capture + FFT-YIN DSP (pitch), toneValid thresholding, pitch frame streaming, connection UX.
...
**Songs live on the phones.** The Android companion owns access to its configured songs folder. The TV does not store or own song files. When the phone connects, it waits for `requestSongList`, scans its folder, and returns metadata. The TV aggregates the library across all phones.

When a song is needed, the Android app's HTTP server streams files directly from SAF URIs (see Section 7). The phone MUST provide stable URLs per connection and keep the HTTP server running until disconnect.
...
**Consequence**: If the Android phone disconnects, all songs it provided disappear from the TV library until it reconnects and re-advertises them. The Android app MUST tear down its HTTP server on disconnect to release SAF resources.
...
Each phone app has a single configured songs folder — a SAF tree on device storage that contains all song subdirectories. The user sets this folder once in the Android app settings. The phone scans this folder recursively for `.txt` files and makes them available to the TV.

#### 3.1.1 Scan implementation (Legacy §3.1.1)
Scanning requires platform-specific file enumeration.

##### 3.1.1.1 Android (SAF — Kotlin)
The songs folder is selected via `ActivityResultContracts.OpenDocumentTree()` and represented as a persisted SAF tree URI (`content://...`). `java.io.File` cannot traverse SAF URIs. Recursive listing MUST use `DocumentFile.fromTreeUri(context, uri).listFiles()` directly (the `DocumentFile` API is part of `androidx.documentfile:documentfile`, already available via `androidx.core`). Recursion depth is bounded by folder structure; no artificial limit is required.

For each `.txt` file found:
- Read contents via `contentResolver.openInputStream(uri)`.
- Parse header tags, resolve asset filenames to SAF URIs via `DocumentFile.findFile(name)`.
- Check file availability with `DocumentFile.exists()`.
- Build HTTP URLs (`coverUrl`, `audioUrl`, etc.) from the on-device HTTP server scheme (see Section 7).
- Preserve modified timestamp via `DocumentFile.lastModified()` for `modifiedTimeMs`.

#### 3.1.2 Song file delivery (Legacy §3.1.2)
The Android app runs a lightweight read-only HTTP server for the duration of its session connection (see Section 7). Song files are served directly from the SAF tree via content resolver streams; no copying to app-private storage. The TV fetches files on demand using URLs provided in `songListUpdate`. The Android app MUST keep the HTTP server alive while connected and stop it on disconnect to release SAF grants.

#### 3.1.3 Phone-side state (Legacy §3.1.3)
The Android app maintains in-memory metadata for songs it has scanned during the current session. This cache is invalidated when:
...
No persistence is required between app launches; scans always reflect the SAF folder at request time.

### 3.2 Discovery and Validation Rules (Legacy §3.2)
**Tests:** F01, F02, F04

#### 3.2.1 Phone-side discovery (normative) (Legacy §3.2.1)
The Android app scans for **all `.txt` files recursively** under its configured songs folder. Each `.txt` produces one song entry, even if multiple `.txt` files exist per directory. Duplicate titles are allowed; identity is `clientId + relativeTxtPath`.
...
- The audio reference resolved by Section 4.2 MUST resolve to an existing SAF file relative to the `.txt` directory (subpaths allowed). Absolute URIs unsupported by the HTTP server count as missing.
...
- For cloud-only placeholders (SAF document without local bytes), the Android app MUST attempt to open a stream. `FileNotFoundException` counts as missing.

#### 3.2.4 MVP parity requirements (Legacy §3.2.4)
- Mirror the recursive `.txt` discovery behavior exactly.
- Reject songs missing required header fields or required audio file.
- Keep invalid song diagnostics (error line number + reason) for export/troubleshooting via developer UI/logcat.
...
This section defines the normative implementation for the on-device pitch detector. The Android companion app MUST implement a custom Fast YIN (FFT-YIN) pipeline.
...
- `fftComplexBuffer`: 4096 floats (for in-place FFT interleaved real/imaginary parts)
...
- **Songs folder**: displays the currently configured songs folder path; pressing OK opens the platform folder picker (`ActivityResultContracts.OpenDocumentTree()`) to change it. On selection, the phone immediately triggers a rescan and sends `songListUpdate` to the TV if connected.
...
The phone app MUST acquire a `WifiManager.MulticastLock` (tag: `"karaoke_multicast"`) when performing mDNS browsing and release it when discovery completes.
...
- File reads use `contentResolver.openAssetFileDescriptor(uri, "r")`.
- File size is obtained via `contentResolver.query(uri, arrayOf(OpenableColumns.SIZE), ...)`.
- Range reads: open the `AssetFileDescriptor`, obtain a `FileInputStream`, skip to `offset`, read `length` bytes.

## 8. Appendices

### A.1 Android Companion App (Kotlin) Dependencies

| Library | Version | Purpose |
|---|---|---|
| `androidx.documentfile:documentfile` | bundled | SAF DocumentFile API |
| `io.ktor:ktor-server-cio` | 2.3.12 | HTTP server |
| `io.ktor:ktor-server-partial-content` | 2.3.12 | Range request support |
| `org.jetbrains.kotlinx:kotlinx-coroutines-android` | 1.7.3 | Coroutines for DSP pipeline |

### A.2 Prohibited Patterns

- Do NOT use `java.io.File` to access SAF URIs.
